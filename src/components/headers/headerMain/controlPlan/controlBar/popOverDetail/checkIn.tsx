...
const CheckIn = () => {
  ...
  return (
    <div className="w-full h-full bg-white rounded-2xl pointer-events-auto" id="checkin-popup">
      <div className="p-8 w-full h-full flex items-center justify-center">
        <div className="w-full h-full">
          <div className="flex items-center justify-between">
            <p className="font-semibold text-xl">{format(firstDayOfMonth, 'MMMM yyyy')}</p>
            <div className="flex items-center justify-evenly gap-6 sm:gap-10">
              Left
              Right
            </div>
          </div>
          <hr className="my-6" />
          <div className="grid grid-cols-7 gap-6 sm:gap-10 place-items-center">
            {days.map((day, idx) => {
              return (
