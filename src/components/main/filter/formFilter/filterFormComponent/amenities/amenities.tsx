...
const Amenities = ({typeAmenities}: AmenitiesProps) => {
  ...
  const handleOnClick = (event: any, item: string) => {
    ...
    if (arrTemp.includes(item)) {
      ...
      setFilterForm({...filterForm, amenities: {...filterForm.amenities, [typeAmenities]: updateArrTemp}});
      return;
    }
    arrTemp.push(item);
    setFilterForm({...filterForm, amenities: {...filterForm.amenities, [typeAmenities]: arrTemp}});
  };
    ...
    <div className='w-full mb-14'>
      ...
      <div className="w-full h-fit grid grid-cols-2 gap-y-5 mt-3">
        {arrAmenities_[typeAmenities].map((item: string, index: number) => {
          return (
            <div className="w-full" key={index} onClick={(event) => handleOnClick(event, item)}>
